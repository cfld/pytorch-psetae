...
def get_data(data_dir, save_dir, bands, dates, geos, tt='train'):

    save_dir = os.path.join(save_dir, tt)
    ...
    for idx_geo, geo in enumerate(geos):
        ...
        train_test = np.array(pd.read_csv(f'{data_dir}/{geo}/FieldIds.csv')[tt])
    ...
    meanstds = [np.mean(X, axis=0) + 1e-7, np.std(X, axis=0) + 1e-7]
...
if __name__ == '__main__':
    ...
    if os.path.isdir(save_dir) == False:
        os.mkdir(save_dir)
    if os.path.isdir(os.path.join(save_dir, 'train' 'META')) == False:
        os.mkdir(os.path.join(save_dir, 'train' 'META'))
    if os.path.isdir(os.path.join(save_dir, 'test' 'META')) == False:
        os.mkdir(os.path.join(save_dir, 'test' 'META'))
    ...
    with open(os.path.join(save_dir, 'train', 'META', 'dates.json'), 'w') as file:
        json.dump(dates_dict, file)
    with open(os.path.join(save_dir, 'test', 'META', 'dates.json'), 'w') as file:
        json.dump(dates_dict, file)

    get_data(cv4a_dir, os.path.join(save_dir), bands=bands, dates=dates, geos=geos, tt='train')
    get_data(cv4a_dir, os.path.join(save_dir), bands=bands, dates=dates, geos=geos, tt='test')